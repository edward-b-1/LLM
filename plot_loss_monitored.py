...
def main():
    ...
    for run_name in args.run_names:
        ...
        if not args.no_val:
            ...
            if "sft_val_loss" in df.columns:
                val = df[df["sft_val_loss"].notna() & (df["sft_val_loss"] != "")]
                ax.plot(val["step"], val["sft_val_loss"].astype(float),
                        label=f"{run_name} sft val", linewidth=2)
            if "wiki_val_loss" in df.columns:
                val = df[df["wiki_val_loss"].notna() & (df["wiki_val_loss"] != "")]
                ax.plot(val["step"], val["wiki_val_loss"].astype(float),
                        label=f"{run_name} wiki val", linewidth=2, linestyle="--")
