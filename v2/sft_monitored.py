...
@torch.no_grad()
def evaluate_sft(model, val_loader, eval_steps, device):
    model.eval()
    losses = []
...
def train(sft_cfg=None, pretrained_checkpoint=None, fresh=False):
    ...
    wiki_val_ds     = TokenDataset(VAL_DATASETS["wikipedia-en"], ModelConfig().context_length)
    wiki_val_loader = DataLoader(wiki_val_ds, batch_size=sft_cfg.batch_size,
                                 shuffle=False, num_workers=2, pin_memory=True)
    ...
    if log_mode == "w":
        log_writer.writerow(["step", "train_loss", "sft_val_loss", "wiki_val_loss", "lr"])
    ...
    # Evaluate at step 0 to record baseline before any SFT
    if step == 1:
        sft_val_loss  = evaluate_sft(model, val_loader,      sft_cfg.eval_steps, device)
        wiki_val_loss = evaluate_clm(model, wiki_val_loader, sft_cfg.eval_steps, device)
        print(f"step     0 | sft val {sft_val_loss:.4f}  wiki val {wiki_val_loss:.4f}  (baseline)")
        log_writer.writerow([0, "", f"{sft_val_loss:.4f}", f"{wiki_val_loss:.4f}", ""])
        log_file.flush()
    ...
    while step <= sft_cfg.max_steps:
        ...
        optimizer.zero_grad()
        accum_loss = 0.0
        for _ in range(sft_cfg.grad_accum_steps):
            try:
            ...
            accum_loss += loss.item() / sft_cfg.grad_accum_steps
        ...
        if step % sft_cfg.log_interval == 0:
            print(f"step {step:5d} | train loss {accum_loss:.4f}  lr {lr:.2e}")
        ...
        if is_eval_step or is_last_step:
            sft_val_loss  = evaluate_sft(model, val_loader,      sft_cfg.eval_steps, device)
            wiki_val_loss = evaluate_clm(model, wiki_val_loader, sft_cfg.eval_steps, device)
            print(f"{'':>8} | sft val  {sft_val_loss:.4f}  wiki val {wiki_val_loss:.4f}  ← step {step}")

        log_writer.writerow([step, f"{accum_loss:.4f}",
                             f"{sft_val_loss:.4f}"  if sft_val_loss  is not None else "",
                             f"{wiki_val_loss:.4f}" if wiki_val_loss is not None else "",
                             f"{lr:.2e}"])
        log_file.flush()
...
if __name__ == "__main__":
    ...
    if args.grad_accum_steps is not None: sft_cfg.grad_accum_steps = args.grad_accum_steps
